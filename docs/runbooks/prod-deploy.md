...
Rules の `activeUser()` ヘルパが書込系すべてに `exists(users/{uid})` を要求しているため、**Firestore レコード削除だけで実質的なアカウント無効化が完了**する。Auth アカウントの削除は完全クリーンアップ用途のオプション。

#### 通常運用（管理 UI 1 操作）

1. https://aidev-knowledge-hub.pages.dev で管理者ログイン → 管理 → ユーザー タブ
2. 対象ユーザの「削除」ボタン押下 → 確認ダイアログで「削除する」
3. 自動で `users/{uid}` + `users/{uid}/private/profile` を atomic 削除（段階 1）→ `handles/{handle}` 解放（段階 2）
4. `admin_logs` に `action: 'delete_user'` で記録される

これだけで:
- 削除済みユーザの既発行 idToken（最大 1h 有効）でも書込が一切通らない（`activeUser()` が `users/{uid}` 不在で拒否）
- 削除済みユーザがログインしても `'profileMissing'` 画面に隔離されるだけで何も操作できない
- 投稿コンテンツ（links / notes / apps / questions / comments）は残置（`createdBy` orphan 参照）
- handle は解放されるので別ユーザが取得可能、ただし Auth アカウントが残るため**同じメアドでの再 signup は不可**

#### 完全クリーンアップ（オプション）

メアドを再利用したい場合、または Auth アカウント自体を消したい場合のみ:

- [Authentication → Users](https://console.firebase.google.com/project/aidevknowledge-app/authentication/users) で対象アカウントの ⋮ → 「ユーザーを削除」
- 順序は問わない（Firestore 削除が先でも後でも安全）

#### 制約

- **最後の管理者は削除不可**（クライアント側でガード）
- **自分自身は削除不可**（Rules で `request.auth.uid != uid` を強制）
- **生きているユーザの handle は管理者でも誤削除不可**（Rules が対応する `users/{uid}` 不在を要求）
